...
public class Player extends MapObject {
    ...
    public void update() {
        ...
        if(firing && currentAction != FIREBALL) {
            if(fire > fireCost) {
                fire -= fireCost;
                FireBall fb = new FireBall(platformMap, facingRight);
                fb.setPosition(x, y);
                fireBalls.add(fb);
            }
        }
    ...
    public boolean checkLevelEnd(LevelEndObject lvlEnd) {


        Rectangle r1=getRectangle();
        Rectangle r2=lvlEnd.getRectangle();

        if (Math.abs(r1.x-r2.x)<3) {
            lvlEnd.setFlinching();
            return true;
        }
        return false;
